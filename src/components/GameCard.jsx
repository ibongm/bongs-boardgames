  ...
  return (
    <article className="rounded-2xl border border-gold/25 bg-walnut p-5 shadow-table flex flex-col min-w-[260px] sm:min-w-0">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {featured && <span className="text-[10px] uppercase tracking-wide bg-gold/20 text-gold px-2 py-0.5 rounded-full">Featured</span>}
        {fresh && <span className="text-[10px] uppercase tracking-wide bg-cream/10 text-parchment px-2 py-0.5 rounded-full">New</span>}
      </div>
      <p className="font-display text-2xl text-gold">{copy?.title || game.meta.title}</p>
      {copy?.similarTo && <p className="text-cream/50 text-sm mt-0.5">Similar to {copy.similarTo}</p>}
      ...
      {hint && <p className="text-cream/55 mt-1 text-xs">{hint}</p>}
      <div className="mt-4 flex flex-col sm:flex-row gap-2">
        <Link to={`/play/${id}`} className="flex-1 text-center bg-gold text-ink font-semibold rounded-md px-4 py-3 min-h-11">
          Play vs bot
        </Link>
        <Link to={`/games/${id}`} className="flex-1 text-center border border-gold/40 rounded-md px-4 py-3 min-h-11 text-cream">
          Tables & rules
        </Link>
      </div>
    </article>
